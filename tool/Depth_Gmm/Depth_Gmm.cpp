...
void Depth_Gmm::saveBgmodel(Rect _box,Mat _image, OutputArray _fgmask, double learningRate,int num,String save_name)
{
    ...
    if( _image.type() == CV_32FC1 )
        processdepth(_image, fgmask, learningRate, bgmodel, nmixtures, backgroundRatio, varThreshold, noiseSigma );
         ...
         cout<< "Only CV_32FC1 images are supported " <<endl;
}
...
void Depth_Gmm::run(Rect _box,Mat _image, OutputArray _fgmask, Mat _bgmodel)
{
    bool needToInitialize = nframes == 0  || _image.size() != frameSize || _image.type() != frameType;
