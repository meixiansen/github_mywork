...
public:
    ...
    void BuildGMMs(Mat& sampleDf, Mat& component1i, Mat& w1f = Mat());
    int RefineGMMs(Mat& sampleDf, Mat& components1i, Mat& w1f = Mat(), bool needReAssign = true); // Iteratively refine GMM

    bool Save(string &name) const;
    bool Load(string &name);
	double GetSumWeight() const {return _sumW;}

    void GetProbs(Mat sampleDf, vector<Mat> &pci1f) const; // Get Probabilities of each Channel i
    void GetProbsWN(Mat sampleDf, vector<Mat> &pci1f) const; // Get Probabilities of each Channel i, without normalize

    void iluProbs(Mat sampleDf, string &nameNE) const; // Get Probabilities of each Channel i, and illustrate it, without normalize
    void iluProbsWN(Mat sampleDf, string &nameNE) const; // Get Probabilities of each Channel i, and illustrate it, without normalize
...
protected:
    ...
    void AssignEachPixel(Mat& sampleDf, Mat &component1i);
};

class MGMM : public CmGMM_<4>{
public:


    MGMM(int K = 5, double thrV = 0.01):CmGMM_<4>(K, thrV) {}
	
    void View(string &title, bool decreaseShow = true);

	// Show foreground probabilities represented by the GMMs
    static double ViewFrgBkgProb(const MGMM &fGMM, const MGMM &bGMM, string &title);

    static void GetGMMs(string &smplW, string &annoExt, MGMM &fGMM, MGMM &bGMM);

    static void Demo();

	// Show GMM images
    void Show(Mat& components1i, string& title);
...
public:
    ...
    template<typename T>  void Add(const T* _c, T _weight);
...
// Add a color sample
template <int D> template<typename T> __forceinline void CmGaussianFitter<D>::Add(const T* _c)
{
	double c[D];
	for (int i = 0;  i < D; i++)
		c[i] = _c[i];
...
template <int D> template<typename T> inline void CmGaussianFitter<D>::Add(const T* _c, T _weight)
{
    double c[D];
	for (int i = 0;  i < D; i++)
		c[i] = _c[i];
...
template <int D> void CmGMM_<D>::BuildGMMs(Mat& sampleDf, Mat& component1i, Mat& w1f)
{
...
//    cout<<"build GMMs "<<endl;
//    for (int j = 0; j < _K; j++) printf("G%d = %g ", j, _Guassians[j].eValues[0]); printf("\n");
	delete []fitters;
}

template <int D> int CmGMM_<D>::RefineGMMs(Mat& sampleDf, Mat& components1i, Mat& w1f, bool needReAssign)
{
    bool weighted = w1f.data != NULL;
	int rows = sampleDf.rows, cols = sampleDf.cols; { 
		CV_Assert(sampleDf.data != NULL && sampleDf.type() == CV_MAKETYPE(CV_32F,D));
...
template <int D> void CmGMM_<D>::AssignEachPixel(Mat& sampleDf, Mat &component1i)
{
	int rows = sampleDf.rows, cols = sampleDf.cols;
...
template <int D> bool CmGMM_<D>::Save(string &name) const
{
...
template <int D> bool CmGMM_<D>::Load(string &name)
{
...
template <int D> void CmGMM_<D>::GetProbs(Mat sampleDf, vector<Mat> &pci) const
{
	pci.resize(_K);
...
template <int D> void CmGMM_<D>::GetProbsWN(Mat sampleDf, vector<Mat> &pci) const
{
	pci.resize(_K);
	for (int c = 0; c < _K; c++) {// for each component c
		pci[c].create(sampleDf.size(), CV_32F);
...
template <int D> void CmGMM_<D>::iluProbs(Mat sampleDf, string &nameNE) const
{
	vector<Mat> pci;
...
template <int D> void CmGMM_<D>::iluProbsWN(Mat sampleDf, string &nameNE) const
{
	vector<Mat> pci;
