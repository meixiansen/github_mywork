...
void Segment::refinement(const cv::Rect rec, pcl::PointCloud<pcl::PointXYZRGB>::Ptr _cloud,vector<int> labels,Mat roi,int *truelabels1,vector<SPixels> &t_superpixels)
{
...
//                waitKey(0);
        ...
        for(int i=0;i<uncategorized_edge.size();i++)
        {
            ...
            int id = -1;
            int tl = -1;
            for(int j=0;j<model_superpixels.size();j++)
            {
                for(int m=0;m<model_superpixels[j].size();m++)
                {
                    double dis = uncategorized_edge[i].CaculateSimLAB(model_superpixels[j][m]);
                    if(min_dis>dis)
                    {
                        min_dis = dis;
                        id = j;
            ...
            if(id!=-1)
            {
                uncategorized_edge[i].true_label = model_superpixels[id][tl].true_label;
                //                model_superpixels[id].push_back(uncategorized_edge[i]);
            }
        }
        for(int i=0;i<uncategorized_edge.size();i++)
        {
        ...
        Mat isolate_mask(origin_image.size(),CV_8UC1);
        ...
        for(int l=0;l<labels.size();l++)
        {
            ...
            for(int j=0;j<t_superpixels.size();j++)
            {
                if(t_superpixels[j].true_label==labels[l]||t_superpixels[j].true_label==-1)
                {
        ...
        if(isoLate)
        {
            Mat isolate_rec = isolate_mask(rec);
            vector<vector<Point> > contour;
            findcontours(isolate_rec,contour);
            for(int i=0;i<contour.size();i++)
            {
                Mat is_mask(isolate_rec.size(),CV_8UC1);
                is_mask.setTo(0);
                Mat dil_mask(isolate_rec.size(),CV_8UC1);
                dil_mask.setTo(0);
                Mat ad_mask(isolate_rec.size(),CV_8UC1);
                ad_mask.setTo(0);
                ...
                vector<int> ad_label;
                getLabelBymask(ad_mask,truelabels1,ad_label);
                vector<SPixels> issp;
                vector<SPixels> adsp;
                vector<int> true_label;
                vector<int> times;
                getSpByLabel(is_label,t_superpixels,issp);
                getSpByLabel(ad_label,t_superpixels,adsp);
                for(int a=0;a<adsp.size();a++)
                {
                    bool Istaken = false;
                    for(int t=0;t<true_label.size();t++)
                    {
                        if(adsp[a].true_label==true_label[t])
                        {
                            Istaken = true;
                            times[t]++;
                        }
                    ...
                    if(!Istaken)
                    {
                        true_label.push_back(adsp[a].true_label);
                        times.push_back(1);
                    }
                }
                int max = 0;
                ...
                for(int i =0;i<issp.size();i++)
                {
                    for(int t=0;t<t_superpixels.size();t++)
                    {
                        if(t_superpixels[t].label==issp[i].label)
                        {
                            t_superpixels[t].true_label = adsp[max_id].true_label;
                        }
                    }
                }
            }
    ...
    for(int i = 0 ;i < contours.size() ; i++)
    {
        ...
        else
        {
            ...
            if(effective_edgec.size()>0)
            {
                ...
                refinement(reca,cloud,labels,roi,truelabels1,t_superpixels);
            }
            else
            {
                Object obj;
