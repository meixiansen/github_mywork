...
void SLIC::DrawContoursAroundSegments(
    ...
    vector<int>&            ccontourx,
    vector<int>&            ccontoury)
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    int number=contourx.size();
    for(int i=0;i<number;i++)
    {
        if(contourx[i]==0 && contoury[i]==0)
            continue;
        else
        {
            ccontourx.push_back(contourx[i]);
            ccontoury.push_back(contoury[i]);
        }
    }
    //////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        int ii = width*height+contoury[j]*width + contourx[j];
        ubuff[ii] = 255;
...
void SLIC::slicsave(
    ...
    const int&				width,
    const int&				height,
    vector<double>&         kkseedsx,
    vector<double>&         kkseedsy,
    vector<int>&            ccontourx,
    vector<int>&            ccontoury,
    vector< vector<int> >&  seedvt,
    vector< vector<int> >&  labelvt)
{
    unsigned long int number1=kkseedsx.size();
    unsigned long int number2=ccontourx.size();
    vector< vector<int> > aa;
    for(int i=0;i<number1;i++)
    {
        vector<int> a;
        int sx=0.5+kkseedsx[i];
        int sy=0.5+kkseedsy[i];
        a.push_back(sx);
        a.push_back(sy);
        for(int j=0;j<number2;j++)
        {
            if(labels[(sy)*width+sx]==labels[ccontoury[j]*width+ccontourx[j]])
            {
                a.push_back(ccontourx[j]);
                a.push_back(ccontoury[j]);
            }
            else
                continue;
        }
    ...
    vector<double>&             kkseedsy)
{
    ...
    {
        kkseedsx.push_back(kseedsx[i]);
        kkseedsy.push_back(kseedsy[i]);
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    vector<double>&             kkseedsy)
{
    ...
    DoSuperpixelSegmentation_ForGivenSuperpixelSize(ubuff,width,height,klabels,numlabels,superpixelsize,compactness,kkseedsx,kkseedsy);
}
