...
//            colors.push_back(c);
//        }
//        Mat save(roi_mask.size(),CV_8UC3);
//        save.setTo(0);
...
void Segment::refinement(const cv::Rect rec,Mat roi_mask ,pcl::PointCloud<pcl::PointXYZRGB>::Ptr _cloud,vector<int> labels,Mat roi,int *truelabels1,vector<SPixels> &t_superpixels)
{
    ...
    for(int i=0;i<t_superpixels.size();i++)
    {
        if( t_superpixels[i].true_label==0||t_superpixels[i].true_label== 0 )
        {
...
//        waitKey(0);
            vector<SPixels>().swap(uncategorized_edges);
}
...
//            waitKey(0);
            ...
            vector<int>().swap(ccontourx1);
            vector<int>().swap(ccontoury1);
            vector<SPixels>().swap(b_superpixels);    //big
            vector<SPixels>().swap(t_superpixels);    //all
        }
    }
