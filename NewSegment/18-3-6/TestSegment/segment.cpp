...
void Segment::getStructElementSize(Mat roi_mask,vector<SPixels> b_superpixels,int* truelabels1,Mat &KedgeR,double &possible)
{
    for(int i=0;i<4;i++)
    {
        ...
        //        imshow("superpixel_region",superpixel_region);
...
void Segment::getEffectEdge(Mat adhere_im,Mat roi_mask,int *truelabels1,vector<SPixels> b_superpixels,vector<SPixels> t_superpixels,vector< vector<Point> > &effective_edgec)
{
    ...
    if(!adhere_cues.empty())
    {
        ...
        for(int i=0;i<edge_sps.size();i++)
        {
            ...
            {
                //                imshow("sps",sps);
                //                imwrite("keyregion2.png",sps);
                //                imshow("KedgeR",KedgeR);
                //                waitKey(0);
                //                                imshow("roi_mask",roi_mask);
                //                                waitKey(0);
                vector< vector<Point> > effect_edge_sps;
                findcontours(KedgeR,effect_edge_sps);
                //                if(!effect_edge_sps.empty()){
...
Mat Segment::refinement(const cv::Rect rec,pcl::PointCloud<pcl::PointXYZRGB>::Ptr _cloud,Mat roi,int *truelabels1,vector<SPixels> &t_superpixels)
{
    ...
    for(int c=0;c<edge_contours.size();c++)
    {
        ...
        //            }
        //        }
        //        imshow("model2.png",save);
        //        waitKey(0);
        //        imshow("tue_mask.png",tue_mask);
        //        waitKey(0);
...
void Segment::processSingle(vector<int> labels,Mat roi_mask,Rect rec,Mat roi,Mat label_mask,int *truelabels1,vector<SPixels> &t_superpixels)
{
    //    cout<<"in"<<endl;
...
void Segment::runSegemnt(){
    ...
    //    foreground.setTo(0);
    //    Mat pic_gray;
    //    cv::cvtColor(origin_image,pic_gray,cv::COLOR_RGB2GRAY);
    //    threshold(pic_gray,foreground,1,255,cv::THRESH_BINARY);
    vector<vector<Point> > contours;
    ...
    for( i=0 ;i < contours.size() ; i++)
    {
        ...
        else if(rec.width*rec.height>2400)
        {
            ...
            if(effective_edgec.size()>0)
            {
                ...
                label_num= getObjects_com(truelabels1,obj_masks,m_mask,b_superpixels);
                if(label_num<2) {
//                    imshow("roi_mask",roi_mask);
//                    imshow("m_mask",m_mask);
//                    imshow("obj_masks",obj_masks);
//                    waitKey(0);
//                    printf("error before crf segment \n");
//                    break;
                    Mat r_mask(origin_image.size(),CV_8UC1);
                    r_mask.setTo(0);
                    for(int i=0;i<roi_mask.rows;i++)
...
//                    break;
                ...
                Mat roi_depth = depth_image(reca);
...
//                waitKey(0);
             ...
             else
             {
                ...
                processSingle(labels,roi_mask,reca,roi,crf_result,truelabels1,t_superpixels);
             }
                }
                //                 cout<<"here2"<<endl;
                //                Mat b_mask(region_mask.size(),CV_8UC1);
                //                for(int i=1;i<=label_num;i++)
                //                {
                //                    b_mask.setTo(0);
                //                    for(int y=0; y<crf_result.rows; y++)
                //                    {
                //                        uchar* triV = crf_result.ptr<uchar >(y);
                //                        uchar* ptr = b_mask.ptr<uchar >(y+reca.y)+reca.x;
                //                        for(int x=0; x<crf_result.cols; x++)
                //                        {
                //                            if(int(triV[x])==i) ptr[x]=255;
                //                        }
                //                    }
                //                    Object obj;
                //                    obj.mask = b_mask.clone();
                //                    objs.push_back(obj);
                //                }
                //                cout<<"here2"<<endl;
                //                                vector<int> labels;
                //                                setSuperpixelsLabels(truelabels1,roi_mask,effective_edgec,t_superpixels,b_superpixels,labels);
                //                                cout<<"setSuperpixelsLabels over"<<endl;
                //                                Mat label_mask = refinement(reca,cloud,roi,truelabels1,t_superpixels);
                //                                     cout<<"refinement over"<<endl;
                //                                 processSingle(labels,roi_mask,reca,roi,label_mask,truelabels1,t_superpixels);
            }
            else
            {
                Object obj;
